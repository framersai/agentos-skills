...
Skills are **SKILL.md files** — each containing YAML frontmatter (metadata) and a markdown body (instructions for the AI agent). They live inside the `@framers/agentos-skills` package as a single NPM data package. There are no individual packages per skill.
...
Fork [framersai/agentos-skills](https://github.com/framersai/agentos-skills) and clone it locally.

```bash
git clone https://github.com/<your-username>/agentos-skills.git
cd agentos-skills
```
